...
- Remarcar aula recorrente cria uma "lápide" cancelada no slot original (NovaAula) — é o que impede a materialização de duplicar a aula. Não remover.
- **Política de faturamento da Play:** `lib/plataforma.js` marca `mc_plataforma_twa` quando o app abre via `android-app://` (TWA). Com a marca ativa, Perfil, gate do 6º aluno e `/assinar` ESCONDEM a venda da assinatura (Mercado Pago) e mostram texto neutro "gerencie pelo site" — obrigatório para não ser rejeitado/removido da Play. Páginas públicas sem login (exigência da Play): `/privacidade` e `/excluir-conta` (early-return no App.jsx antes do guard de sessão).
